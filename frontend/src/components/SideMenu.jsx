...
if(params.productId){
  ...
  >
      ...
      {isSubcategoryPage ?  <FiltersnCategory priceRange={priceRange} discounts={discounts} brands={brands}setBrands={setBrands} setDiscounts={setDiscounts} setPriceRange={setPriceRange} discountOptions={discountOptions} brandOptions={brandOptions} menu={menu}/>:
          ...
          <div className="hidden group-hover:block absolute left-full top-0 w-80 bg-white shadow-md rounded-lg p-4 z-20">
            {option.subcategories.map((subcategory, subIndex) => (
              <div
                key={subIndex}
                onClick={() => {
                  navigate(`/categories/${option.slug}/${subcategory.slug}`);
                  setShowSideMenu(false);
                }}
...
function FiltersnCategory({discountOptions,brandOptions,priceRange,discounts,brands,setBrands,setDiscounts,setPriceRange,menu}) {
  ...
  return (
    ...
    <Accordion className='w-full bg-white rounded-md'>
        ...
        >
          <p className='!text-xs md:!text-sm !text-blue-900 font-semibold'>Go to Categories</p>
        </AccordionSummary>
        <AccordionDetails>
          <div className="md:w-80 bg-white shadow-md rounded-lg p-4 overflow-y-auto md:overflow-y-visible h-full">
    {menu?.slice(
  ...
  <AccordionSummary
    ...
    id="panel2-header"
  >
    <p className="!text-xs md:!text-sm !text-blue-900 font-semibold">Filters</p>
  </AccordionSummary>
  ...
  <AccordionDetails>
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      {/* Price Range Filter */}
      <div>
        <label className="!text-xs md:!text-sm font-medium text-gray-700 mb-2 block">Price Range:</label>
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          <span className="!text-xs md:!text-sm text-gray-500 px-1">to</span>
      ...
      {/* Discount Filter */}
      <div>
        <label className="!text-xs md:!text-sm font-medium text-gray-700 mb-2 block">Discount:</label>
      ...
      {/* Brand Filter */}
      <div>
        <label className="!text-xs md:!text-sm !font-medium text-gray-700 mb-2 block">Brand:</label>
        <div className="flex flex-col gap-2 text-sm px-1 text-gray-600">
