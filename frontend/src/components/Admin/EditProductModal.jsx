...
const EditProductModal = ({ isOpen, onClose, productData, onSave, menu }) => {
  ...
  useEffect(() => {
    ...
    console.log(`Product recieved`, productData);

    // console.log("category value", category);
    if(productData && productData.category){
      setCategory(
      menu.filter((category) => category._id == productData.category._id)[0]
    );
    }
    ...
    if(productData?.partCod)
      setDeliveryType((prevData)=>[...prevData, "PARTCOD"]);
    if(productData?.prepaid)
      setDeliveryType((prevData)=>[...prevData, "PREPAID"]);
    if(productData?.return)
      setDeliveryType((prevData)=>[...prevData, "RETURN"]);

    if(productData?.returnTime)
      setReturnDays(productData?.returnTime);

    if(productData?.isActive)
    setStatus(productData?.isActive);
  ...
  const [category, setCategory] = useState("");
   ...
   useEffect(() => {
    ...
    // console.log("Subcategory in product", productData.subCategory);
    setSubcategory(
      category?.subcategories?.filter(
      (subcategory) => subcategory._id == productData.subCategory._id
    )[0]
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-auto p-4">
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[95%] overflow-auto p-6 shadow-lg">
        ...
        <h2 className="text-md md:text-lg font-semibold mb-4">Edit Product</h2>
        <i className="ri-close-line cursor-pointer text-blue-950 text-2xl" onClick={onClose}></i>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 ">
          ...
          {category && category.subcategories && category.subcategories.length > 0 && subcategory && <FormControl fullWidth>
            <label className="text-sm font-medium text-gray-700 mb-1">
              Subcategory
            </label>
            <Select
