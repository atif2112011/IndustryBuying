...
const updateUserAddress = async (req, res,next) => {
  try {
    ...
    if (!addressToUpdate) 
      throw new Error("Address not found with given ID");
...
const unBlockUser = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ message: "User UnBlocked", user: user,success:true });
  } catch (error) {
    next(error);
  }
};

module.exports = { updateUserProfile, getUserProfile,updateUserAddress,addUserAddress,deleteUser,blockUser,unBlockUser };
