...
router.get("/auth/google/callback", async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
    const token=jwt.sign({userId: existingUser._id, role: existingUser.role}, process.env.JWT_SECRET, {expiresIn: "15m"});
    ...
    res.redirect(`http://localhost:5173/user/register?token=${token}`);
    }
